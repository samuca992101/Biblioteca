...
export class LivroController {
 listar = async(req: Request, res: Response) => {
  const livros = await livroRepository.find({ relations: ["autores", "editora"] });
  res.json(livros);
}

  criar = async (req: Request, res: Response) => {
  const { titulo, anoPublicacao, genero, autorIds, editoraId } = req.body;
  ...
  if (!editora) {
    return res.status(400).json({ mensagem: "Editora não encontrada" });
  }
...
atualizar = async(req: Request, res: Response) => {
  const id = Number(req.params.id);
  const { titulo, genero, anoPublicacao, autorIds, editoraId } = req.body;
